...
rasterizeMatrix <- function(data, pos, bbox, resolution = 100, square = TRUE, fun = "mean", n_threads = 1, BPPARAM = NULL, verbose = TRUE) {
  ...
  stopifnot("bbox must be of class 'numeric' or 'bbox' (as generated by sf::st_bbox)" = (is(bbox, "bbox") | is(bbox, "numeric")))
  if (is(bbox, 'numeric')) {
...
permutateByRotation <- function(input, n_perm = 1, verbose = FALSE) {
  ...
  angles <- seq(0, 360, by = 360/n_perm)[seq_len(n_perm)]
  ...
  if (is.list(input)) {
    ...
    output <- unlist(lapply(input, function(spe) {
      stopifnot("input must be a SpatialExperiment object or a list of SpatialExperiment objects"=is(spe, "SpatialExperiment"))
    ...
    stopifnot("input must be a SpatialExperiment object or a list of SpatialExperiment objects"=is(input, "SpatialExperiment"))
